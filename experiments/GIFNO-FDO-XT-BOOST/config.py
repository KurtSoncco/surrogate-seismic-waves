# config.py
"""Configuration for GIFNO-FDO-XT-BOOST: band-targeted TF residual boost + position-aware 2D DeepONet trunk."""
...
MODEL_SAVE_DIR = Path(
    os.environ.get("GIFNO_MODEL_DIR", TF_RESULTS_DIR / "models" / "fdo_xt_boost")
)
RESULTS_SAVE_DIR = Path(
    os.environ.get("GIFNO_RESULTS_DIR", TF_RESULTS_DIR / "results" / "fdo_xt_boost")
)
...
# --- SpecBoost-style residual corrector ---
PRETRAIN_CHECKPOINT: str = os.environ.get("GIFNO_PRETRAIN_CHECKPOINT", "")
BOOST_ETA: float = 0.1
BOOST_PRED_HIDDEN: int = 128
BOOST_BAND_WEIGHT_LOW: float = 0.25
BOOST_BAND_WEIGHT_MID: float = 1.0
BOOST_BAND_WEIGHT_HIGH: float = 1.0
...
# --- W&B ---
WANDB_PROJECT: str = "gifno_fdo_xt_boost"
WANDB_RUN_NAME: str = "boost_run"
...
def _parse_env_value(key: str, raw: str):
    ...
    if key in (
        "BOOST_ETA",
        "BOOST_PRED_HIDDEN",
        "BOOST_BAND_WEIGHT_LOW",
        "BOOST_BAND_WEIGHT_MID",
        "BOOST_BAND_WEIGHT_HIGH",
        "LEARNING_RATE",
        "WEIGHT_DECAY",
        "LOSS_REL_WEIGHT",
        "LOSS_H1_WEIGHT",
...
def apply_env_overrides() -> list[str]:
    ...
    if applied:
        print(
            "[GIFNO-FDO-XT-BOOST config] env overrides: " + ", ".join(applied),
            flush=True,
        )
    return applied
...
def setup_import_paths() -> None:
    """Ensure BOOST config is used when importing shared GIFNO modules."""
