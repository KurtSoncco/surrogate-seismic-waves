#!/usr/bin/env python3
"""Submit GIFNO-FDO-XT-BOOST sweep jobs to SLURM."""
...
DEFAULT_VARIANTS_BOOST_TSV = """\
boost_ref	LATENT_CHANNELS=128;DEEPONET_LATENT_DIM=128
...
def load_variants(path: Path) -> list[SweepVariant]:
    ...
    print(
        f"[GIFNO-FDO-XT-BOOST sweep] WARNING: {path} not found — using built-in fallback.",
        file=sys.stderr,
        flush=True,
    )
    return load_variants_from_text(DEFAULT_VARIANTS_BOOST_TSV)
...
def build_export_env(
    ...
    *,
    ...
    sweep_root = "fdo_xt_boost/sweep" if tag == "full" else f"fdo_xt_boost/sweep/{tag}"
    env = {
    ...
    if pretrain := os.environ.get("GIFNO_PRETRAIN_CHECKPOINT"):
        env["GIFNO_PRETRAIN_CHECKPOINT"] = pretrain
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Submit GIFNO-FDO-XT-BOOST sweep jobs")
    parser.add_argument(
        "--variants",
        type=Path,
        default=Path(__file__).resolve().parent / "sweep_variants_boost.tsv",
    )
    ...
    print(f"=== GIFNO-FDO-XT-BOOST sweep: {len(variants)} job(s), mode={mode} ===")
