# train.py
"""Training loop for GIFNO-FDO-XT-BOOST (frozen base + residual head)."""
...
def train_model(train_loader, val_loader):
    ...
    ckpt = config.PRETRAIN_CHECKPOINT.strip()
    if not ckpt:
        raise RuntimeError(
            "BOOST requires GIFNO_PRETRAIN_CHECKPOINT or config.PRETRAIN_CHECKPOINT"
        )
    load_xt_base(model, ckpt)
    freeze_base(model)

    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[BOOST] Trainable parameters: {trainable:,}")
    ...
    t = trange(config.NUM_EPOCHS, desc="Training BOOST")
    for epoch in t:
        model.train()
        model.base.eval()
        train_loss = 0.0
