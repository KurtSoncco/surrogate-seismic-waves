...
def plot_reconstructions(real_profiles, recon_profiles, save_path, num_examples=8):
    """
    ...
    freq = np.loadtxt(TrainingConfig().freq_data_path)

    for i in range(num_examples):
        # --- Plot Real Profile (Left Column) ---
        ax_real = axes[i, 0]
        ax_real.loglog(freq, real_profiles_np[i])
        ax_real.set_title("Real")
        ax_real.set_ylabel("TF Amplitude")
        ax_real.set_xlabel("Frequency (Hz)")
        ax_real.grid(True, linestyle="--", alpha=0.6)

        # --- Plot Reconstructed Profile (Right Column) ---
        ax_recon = axes[i, 1]
        ax_recon.loglog(freq, recon_profiles_np[i])
        ax_recon.set_title("Reconstructed")
        ax_recon.set_xlabel("Frequency (Hz)")
        ax_recon.set_ylabel("TF Amplitude")
        ax_recon.grid(True, linestyle="--", alpha=0.6)

        # Share x-axis with the real plot for consistent depth scaling
        ax_recon.sharex(ax_real)
