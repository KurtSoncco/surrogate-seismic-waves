#!/usr/bin/env python3
"""Submit GIFNO-FDO-XT sweep jobs to SLURM."""
...
DEFAULT_VARIANTS_HFNO_TSV = """\
hfno_ref\tLATENT_CHANNELS=128;DEEPONET_LATENT_DIM=128
hfno_lat96_d128\tLATENT_CHANNELS=96;DEEPONET_LATENT_DIM=128
hfno_lat64_d64\tLATENT_CHANNELS=64;DEEPONET_LATENT_DIM=64
hfno_lat96_d64\tLATENT_CHANNELS=96;DEEPONET_LATENT_DIM=64
hfno_lat96_d192\tLATENT_CHANNELS=96;DEEPONET_LATENT_DIM=192
hfno_depth\tLATENT_CHANNELS=128;DEEPONET_LATENT_DIM=128;BRANCH_MODE=depth
hfno_modes48\tLATENT_CHANNELS=128;DEEPONET_LATENT_DIM=128;FNO_MODES=48,48
"""
...
def load_variants(path: Path) -> list[SweepVariant]:
    ...
    print(
        f"[GIFNO-FDO-XT-HFNO sweep] WARNING: {path} not found — using built-in fallback.",
        file=sys.stderr,
        flush=True,
    )
    return load_variants_from_text(DEFAULT_VARIANTS_HFNO_TSV)
...
def build_export_env(
    ...
    *,
    ...
    sweep_root = "fdo_xt_hfno/sweep" if tag == "full" else f"fdo_xt_hfno/sweep/{tag}"
    env = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Submit GIFNO-FDO-XT-HFNO sweep jobs")
    parser.add_argument(
        "--variants",
        type=Path,
        default=Path(__file__).resolve().parent / "sweep_variants_hfno.tsv",
    )
    ...
    print(f"=== GIFNO-FDO-XT-HFNO sweep: {len(variants)} job(s), mode={mode} ===")
