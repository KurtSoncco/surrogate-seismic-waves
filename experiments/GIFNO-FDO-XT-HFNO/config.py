# config.py
"""Configuration for GIFNO-FDO-XT-HFNO: H-FNO hybrid encoder + position-aware 2D DeepONet trunk."""
...
MODEL_SAVE_DIR = Path(
    os.environ.get("GIFNO_MODEL_DIR", TF_RESULTS_DIR / "models" / "fdo_xt_hfno")
)
RESULTS_SAVE_DIR = Path(
    os.environ.get("GIFNO_RESULTS_DIR", TF_RESULTS_DIR / "results" / "fdo_xt_hfno")
)
...
# --- W&B ---
WANDB_PROJECT: str = "gifno_fdo_xt_hfno"
WANDB_RUN_NAME: str = "hfno_run"
...
def apply_env_overrides() -> list[str]:
    ...
    if applied:
        print(
            "[GIFNO-FDO-XT-HFNO config] env overrides: " + ", ".join(applied),
            flush=True,
        )
    return applied
...
def setup_import_paths() -> None:
    """Ensure HFNO config is used when importing shared GIFNO modules."""
