#!/bin/bash
# GIFNO-FDO training on NCSA Delta.
#
#   cd ~/surrogate-seismic-waves/experiments/GIFNO-FDO
#   source ../GIFNO/delta_env.sh
#   sbatch delta_train.sh
#   bash delta_sweep.sh --limit 2000 --dry-run

#SBATCH --job-name=fdo_train
...
#SBATCH --output=fdo_train.o%j
#SBATCH --error=fdo_train.e%j
...
export GIFNO_MODEL_DIR="${GIFNO_MODEL_DIR:-${GIFNO_TF_DIR}/models/fdo}"
export GIFNO_RESULTS_DIR="${GIFNO_RESULTS_DIR:-${GIFNO_TF_DIR}/results/fdo}"
...
echo "=== GIFNO-FDO training on Delta ==="
echo "WANDB_PROJECT=gifno_fno_deeponet"
