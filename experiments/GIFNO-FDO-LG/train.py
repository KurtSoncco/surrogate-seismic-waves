# train.py
"""Phased transfer-learning training for GIFNO-FDO-LG."""
...
def train_model(train_loader, val_loader):
    ...
    optimizer = build_phase_optimizer(model, config.TRAIN_PHASE)
    ...
    t = trange(config.NUM_EPOCHS, desc=f"LG phase {config.TRAIN_PHASE}")
    for epoch in t:
        model.train()
        train_loss = 0.0
        n_train = 0
        for inputs, targets, masks in train_loader:
            ...
            with autocast:
                ...
                loss = loss + l2sp_penalty(model, anchor, config.L2SP_WEIGHT)
