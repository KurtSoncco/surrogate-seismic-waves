#!/bin/bash
# GIFNO-FDO-LG transfer training on NCSA Delta.
#
#   cd ~/surrogate-seismic-waves/experiments/GIFNO-FDO-LG
#   export GIFNO_PRETRAIN_CHECKPOINT=/path/to/xt_lat128_d128/best_model.pt
#   export GIFNO_TRAIN_PHASE=1
#   source ../GIFNO/delta_env.sh
#   sbatch delta_train.sh

#SBATCH --job-name=lg_train
...
#SBATCH --output=lg_train.o%j
#SBATCH --error=lg_train.e%j
...
export GIFNO_MODEL_DIR="${GIFNO_MODEL_DIR:-${GIFNO_TF_DIR}/models/fdo_lg}"
export GIFNO_RESULTS_DIR="${GIFNO_RESULTS_DIR:-${GIFNO_TF_DIR}/results/fdo_lg}"
...
echo "=== GIFNO-FDO-LG training on Delta ==="
echo "WANDB_PROJECT=gifno_fdo_lg"
