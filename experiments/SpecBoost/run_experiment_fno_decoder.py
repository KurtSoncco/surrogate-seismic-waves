# run_experiment_fno_decoder.py
"""
Experiment 3: Can a model with an FNO Decoder learn the residual signal
effectively from the previous prediction?
"""
...
# NEW: Import the FNO-based model
from experiment_fno_model import ResidualFNOModel
...
logger.name = "SpecBoost_Experiment_FNO_Decoder"
...
def main():
    ...
    # Create datasets and dataloaders (reusing from previous successful experiment)
    ...
    # Initialize the NEW FNO-based residual model
    model = ResidualFNOModel(
        pred_encoder_channels=config.PRED_ENCODER_CHANNELS,
        latent_dim=config.LATENT_DIM,
        output_size=config.OUTPUT_SIZE,
        fno_modes=config.FNO_MODES,  # Use FNO params from config
        fno_width=config.FNO_WIDTH,  # Use FNO params from config
        dropout=config.DROPOUT_RATE,  # Using the standard dropout rate
    )

    # Run the training experiment (reusing the generic trainer)
    run_experiment_training_loop(
        model,
        train_loader,
        val_loader,
        learning_rate=config.BOOSTING_LEARNING_RATE,
        experiment_name="FNO_Decoder_on_Residuals",
    )
