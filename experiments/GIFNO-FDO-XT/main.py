# main.py
"""GIFNO-FDO-XT: FNO encoder + position-aware 2D DeepONet training and evaluation."""
...
def ensure_tf_cache():
    if config.TF_PER_SAMPLE_PATH.exists() and config.MANIFEST_PATH.exists():
        return
    print("[GIFNO-FDO-XT] TF cache not found — running GIFNO preprocessing...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="GIFNO-FDO-XT training pipeline")
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
