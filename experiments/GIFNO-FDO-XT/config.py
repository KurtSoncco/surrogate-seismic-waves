...
# --- FNO encoder ---
# Baseline XT uses 4 channels (Vs, zeta, x, z). With SCALE_SPLIT_VS the loader
# emits 6: (Vs, Vs_macro, Vs_rf, zeta, x, z). IN_CHANNELS is derived below.
LATENT_CHANNELS: int = 96
FNO_MODES: Tuple[int, int] = (32, 32)
NUM_FNO_LAYERS: int = 5
# Dual-path RF branch (only used when DUAL_PATH_ENCODER and SCALE_SPLIT_VS).
FNO_MODES_RF: Tuple[int, int] = (48, 48)
NUM_FNO_LAYERS_RF: int = 4

# --- Scale-split Vs + dual-path encoder (seed-conditional recipe) ---
SCALE_SPLIT_VS: bool = False
VS_MACRO_KERNEL: int = 15  # odd box size for Vs_macro; Vs_rf = Vs - Vs_macro
DUAL_PATH_ENCODER: bool = False

# --- DeepONet readout (2D trunk: log f + x) ---
BRANCH_MODE: str = "surface"  # surface | depth
OUTPUT_ACTIVATION: str = "none"  # none | softplus | exp  (positivity on |TF|)
DEEPONET_LATENT_DIM: int = 128
TRUNK_HIDDEN: int = 128
TRUNK_LAYERS: int = 4
X_COORD_MODE: str = "normalized"  # normalized: x / domain half-width | meters

# --- Seed-contrast training (within Sobol sample_id groups) ---
SEEDS_PER_SAMPLE: int = 30
RETURN_SAMPLE_ID: bool = False
SEED_CONTRAST_WEIGHT: float = 0.0
SEED_SIGMA_LN_WEIGHT: float = 0.0
SEED_GROUP_BATCH: bool = False  # pair replicates from the same sample_id in a batch
...
def _apply_seed_conditional_recipe() -> None:
    """Enable the seed-conditional XT recipe via GIFNO_SEED_CONDITIONAL_RECIPE=1."""
    raw = os.environ.get("GIFNO_SEED_CONDITIONAL_RECIPE", "")
    if raw.lower() not in ("1", "true", "yes", "on"):
        return
    g = globals()
    g["BRANCH_MODE"] = "depth"
    g["OUTPUT_ACTIVATION"] = "softplus"
    g["SCALE_SPLIT_VS"] = True
    g["DUAL_PATH_ENCODER"] = True
    g["RETURN_SAMPLE_ID"] = True
    ...
    print(
        ...
        "depth branch, softplus, scale-split+dual-path, seed-contrast",
        flush=True,
    )


_apply_seed_conditional_recipe()

# Derived channel count (after recipe / before env overrides that may flip flags).
IN_CHANNELS: int = 6 if SCALE_SPLIT_VS else 4
...
def _parse_env_value(key: str, raw: str):
    ...
    if key == "OUTPUT_ACTIVATION":
        mode = raw.lower()
        if mode not in ("none", "softplus", "exp"):
            raise ValueError(
                f"OUTPUT_ACTIVATION must be none|softplus|exp, got {raw!r}"
            )
        return mode
    if key == "X_COORD_MODE":
        mode = raw.lower()
    ...
    if key in (
        ...
        "LR_SCHED_FACTOR",
        "SEED_CONTRAST_WEIGHT",
        "SEED_SIGMA_LN_WEIGHT",
    ):
        return float(raw)
...
def apply_env_overrides() -> list[str]:
    ...
    # Keep IN_CHANNELS consistent unless explicitly overridden.
    if "IN_CHANNELS" not in {a.split("=", 1)[0] for a in applied}:
        g["IN_CHANNELS"] = 6 if g["SCALE_SPLIT_VS"] else 4
    if applied:
