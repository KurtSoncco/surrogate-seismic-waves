...
def test_sweep_variants_xt_loads_nine():
    ...
    seed_cond = next(v for v in variants if v.name == "xt_seed_conditional")
    assert seed_cond.overrides["SCALE_SPLIT_VS"] == "true"
    assert seed_cond.overrides["DUAL_PATH_ENCODER"] == "true"
    assert seed_cond.overrides["SEED_CONTRAST_WEIGHT"] == "0.1"
    assert seed_cond.overrides["OUTPUT_ACTIVATION"] == "softplus"
