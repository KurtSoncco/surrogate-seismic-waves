...
def _band_curriculum_enabled(criterion: torch.nn.Module) -> bool:
    ...
    base = (
        criterion.base if isinstance(criterion, SeedAwareCompositeLoss) else criterion
    )
    return getattr(config, "BAND_CURRICULUM", False) and hasattr(
        base, "set_band_weights"
    )
...
def _unpack_batch(batch):
    """Support (x, y, mask) or (x, y, mask, sample_id) batches."""
    if len(batch) == 4:
...
def train_model(train_loader, val_loader):
    ...
    seed_aware = isinstance(criterion, SeedAwareCompositeLoss)
    ...
    for epoch in t:
        ...
        for batch in train_loader:
            inputs, targets, masks, sample_ids = _unpack_batch(batch)
            ...
            if sample_ids is not None:
                sample_ids = sample_ids.to(config.DEVICE, non_blocking=non_blocking)

            optimizer.zero_grad(set_to_none=True)
            with autocast:
                outputs = model(inputs)
                if sample_ids is not None and seed_aware:
                    loss = criterion(outputs, targets, masks, sample_ids=sample_ids)
                else:
