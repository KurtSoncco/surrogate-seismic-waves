# main.py
"""Main script to run the DeepONet training and evaluation pipeline."""
...
def run_pipeline():
    ...
    # Frequency data is crucial input for the DeepONet Trunk
    freq_data = np.genfromtxt(config.FREQ_PATH, delimiter=",")
    # Reshape frequency data to be (OUTPUT_SIZE, 1) for Trunk's 1D input
    ...
    # Pass frequency data to the data loader
    train_loader, val_loader, test_loader = get_data_loaders(
        vs_list, ttf_list, freq_data, config.BATCH_SIZE
    )

    # --- Train Model ---
    logger.info("Starting DeepONet model training...")
    run = train_model(train_loader, val_loader)

    # --- Evaluate Model ---
    logger.info("Starting DeepONet model evaluation...")
    evaluate_model(
        test_loader, freq_data.squeeze(), run=run
    )  # Pass 1D freq_data for plotting
