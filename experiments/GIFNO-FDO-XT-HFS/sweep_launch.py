#!/usr/bin/env python3
"""Submit GIFNO-FDO-XT-HFS sweep jobs to SLURM."""
...
DEFAULT_VARIANTS_HFS_TSV = """\
hfs_ref	LATENT_CHANNELS=128;DEEPONET_LATENT_DIM=128
...
def load_variants(path: Path) -> list[SweepVariant]:
    ...
    print(
        f"[GIFNO-FDO-XT-HFS sweep] WARNING: {path} not found — using built-in fallback.",
        file=sys.stderr,
        flush=True,
    )
    return load_variants_from_text(DEFAULT_VARIANTS_HFS_TSV)
...
def build_export_env(
    ...
    *,
    ...
    sweep_root = "fdo_xt_hfs/sweep" if tag == "full" else f"fdo_xt_hfs/sweep/{tag}"
    env = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Submit GIFNO-FDO-XT-HFS sweep jobs")
    parser.add_argument(
        "--variants",
        type=Path,
        default=Path(__file__).resolve().parent / "sweep_variants_hfs.tsv",
    )
    ...
    print(f"=== GIFNO-FDO-XT-HFS sweep: {len(variants)} job(s), mode={mode} ===")
