#!/usr/bin/env python3
"""
Evaluate a saved LOGLO-POD checkpoint on the test split; log metrics and plots to W&B.

Example (interactive GPU node):
  export GIFNO_H5_DIR=.../h5
  export GIFNO_TF_DIR=.../transfer_function
  export GIFNO_MODEL_DIR=.../models/fdo_xt_loglo_pod/sweep/n500/loglo_pod_ref
  export GIFNO_LATENT_CHANNELS=128
  export GIFNO_POD_NUM_MODES=32

  python eval_checkpoint.py --limit 500 --wandb-run-id <id>
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate LOGLO-POD checkpoint and log test metrics/plots to W&B"
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Dataset limit (use same value as the sweep, e.g. 500)",
    )
    parser.add_argument(
        "--wandb-run-id",
        type=str,
...
def main() -> int:
    ...
    if not args.no_wandb:
        ...
        if args.wandb_run_name:
            ...
            init_kwargs["name"] = config.WANDB_RUN_NAME or "loglo_pod_eval"
